...
##' @param testNonNormal A function used to perform the nonparametric tests. The default is \code{kruskal.test} (Kruskal-Wallis Rank Sum Test). This is equivalent of the wilcox.test (Mann-Whitney U test) when there are only two groups.
...
##'
##' @details The definitions of the standardized mean difference (SMD) are available in \href{https://www.tandfonline.com/doi/abs/10.1080/00031305.1986.10475403}{Flury \emph{et al} 1986} for the univariate case and the multivariate case (essentially the square root of the Mahalanobis distance). Extension to binary variables is discussed in \href{https://www.tandfonline.com/doi/abs/10.1080/03610910902859574}{Austin 2009} and extension to multinomival variables is suggested in \href{https://support.sas.com/resources/papers/proceedings12/335-2012.pdf}{Yang \emph{et al} 2012}. This multinomial extesion treats a single multinomial variable as multiple non-redundant dichotomous variables and use the Mahalanobis distance. The off diagonal elements of the covariance matrix on page 3 have an error, and need negation. In weighted data, the same definitions can be used except that the mean and standard deviation estimates are weighted estimates (\href{https://pubmed.ncbi.nlm.nih.gov/23902694/}{Li \emph{et al} 2013} and \href{https://onlinelibrary.wiley.com/doi/full/10.1002/sim.6607}{Austin \emph{et al} 2015}). In tableone, all weighted estimates are calculated by weighted estimation functions in the \code{survey} package.
##'
...
CreateFullTableOne <-
  function(vars,                                      # character vector of variable names
    ...
    for(vn in varNumerics){
      if(is.null(maxNumLevel)){
        break
      }
      NumLevel <- length(unique(data[, vn]))
      if(NumLevel > maxNumLevel){
        next
      }
      newVar <- paste0(vn, '__N2F__')
      data[newVar] <- factor(data[, vn])
      varFactors <- c(varFactors, newVar)
      vars <- c(vars, newVar)
    }
